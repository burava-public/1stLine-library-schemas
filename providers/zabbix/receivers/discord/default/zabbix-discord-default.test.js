...
const trustedResolve = await loadSchemaPackage(import.meta.url, {
  schema: 'zabbix-discord-default.json',
  fixture: 'fixtures/trusted-sources/zabbix-release-7.4/trigger-resolve.json',
});
...
await test('keeps incident variables inside lifecycle mappings only', () => {
  ...
  assert.ok(lifecycle.resolved.overrides['embeds[0].description'].includes('${incident.event.actor_name}'));
  assert.ok(lifecycle.resolved.overrides['embeds[0].description'].includes('${incident.metrics.mttr_seconds}'));
